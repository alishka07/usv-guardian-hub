...
import { Bot, ZoomIn, ZoomOut, Layers, Compass, Anchor, Building2, Waves, MapPin } from "lucide-react";
...
export function MapView({ robots, samples, onSelectRobot, onSelectSample, selectedRobotId }: Props) {
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <div className="relative w-full h-full rounded-xl overflow-hidden border border-border bg-[oklch(0.18_0.025_245)]">
        {/* terrain backdrop */}
          ...
          <Tooltip key={r.id}>
            <TooltipTrigger asChild>
              ...
              >
                <div className="relative">
                  ...
                  >
                    <Bot className="size-5" style={{ color: r.status === "online" ? r.color : "oklch(0.7 0.02 250)" }} />
                    {/* Heading indicator */}
                    {r.status === "online" && (
                      <div
            ...
            <TooltipContent side="top">
              <div className="text-xs space-y-0.5">
                ...
                {r.status === "online" && <div className="text-success">В сети · {r.battery.toFixed(0)}% · {r.speed} м/с</div>}
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-1 bg-card/85 backdrop-blur border border-border rounded-lg p-1 shadow-lg">
          <Button size="icon" variant="ghost" onClick={() => setZoom(Math.min(3, zoom + 1))} title="Приблизить"><ZoomIn className="size-4" /></Button>
          <Button size="icon" variant="ghost" onClick={() => setZoom(Math.max(1, zoom - 1))} title="Отдалить"><ZoomOut className="size-4" /></Button>
        ...
        <div className="absolute top-4 right-4 bg-card/85 backdrop-blur border border-border rounded-lg px-3 py-2 text-[11px] font-mono leading-tight">
          ...
          <div className="text-muted-foreground">ZOOM {zoom}/3 · UTC+5 Алматы</div>
        </div>
