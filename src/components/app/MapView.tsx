...
import {
  Bot, ZoomIn, ZoomOut, Layers, Compass, Anchor, Building2, Waves,
  MapPin, Pencil, RotateCcw, Maximize2, Crosshair, Ruler,
} from "lucide-react";
...
export function MapView({
  robots, samples, onSelectRobot, onSelectSample, selectedRobotId,
  editMode, editingRobotId, draftWaypoints, onMapClick, highlightedSampleId,
}: Props) {
    ...
    <TooltipProvider delayDuration={150}>
      <div
        ...
        style={{
          ...
          boxShadow: "inset 0 0 0 1px color-mix(in oklab, var(--cyan-accent) 14%, transparent), var(--shadow-elev)",
        }}
      >
        {/* === Transform layer === */}
        <div className="absolute inset-0" style={transformStyle}>
          ...
          <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
            ...
            <path d={RESERVOIR_PATH} fill="oklch(0.78 0.10 200)" opacity="0.18" filter="url(#soft)"
                  transform="translate(0 0.4) scale(1.02 1.04)" />

            {/* Drop shadow */}
            <path d={RESERVOIR_PATH} fill="oklch(0.10 0.04 238)" opacity="0.6"
                  transform="translate(0.4 0.6)" filter="url(#soft)" />
            ...
            <g clipPath="url(#water-clip)" opacity="0.65">
              <path d="M 18,60 C 36,56 56,49 80,40" fill="none"
                    stroke="oklch(0.88 0.09 200 / 0.35)" strokeWidth="0.18" strokeDasharray="0.9 0.9" />
              <path d="M 22,62 C 42,58 60,52 82,44" fill="none"
                    stroke="oklch(0.88 0.09 200 / 0.25)" strokeWidth="0.16" strokeDasharray="0.6 1.1" />
              <path d="M 28,60 C 44,56 60,50 78,43" fill="none"
                    stroke="oklch(0.88 0.09 200 / 0.18)" strokeWidth="0.14" strokeDasharray="0.4 1.4" />
            </g>
            ...
            <path d={RESERVOIR_PATH} fill="none" stroke="oklch(0.92 0.10 195 / 0.55)" strokeWidth="0.55" filter="url(#soft)" />
            <path d={RESERVOIR_PATH} fill="none" stroke="url(#shoreline)" strokeWidth="0.18" />

            {/* River Ili inlet — gradient + glow */}
            <path d={RIVER_PATH} stroke="oklch(0.62 0.12 215)" strokeWidth="1.5" fill="none"
                  strokeLinecap="round" opacity="0.9" filter="url(#soft)" />
            <path d={RIVER_PATH} stroke="oklch(0.92 0.09 200)" strokeWidth="0.3" fill="none" strokeLinecap="round" />
            {/* Chilik tributary */}
            <path d={TRIB_PATH} stroke="oklch(0.62 0.12 215)" strokeWidth="0.7" fill="none"
                  strokeLinecap="round" opacity="0.75" filter="url(#soft)" />
            <path d={TRIB_PATH} stroke="oklch(0.92 0.09 200)" strokeWidth="0.18" fill="none" strokeLinecap="round" />
            ...
            {showTrails && zoom >= 1.2 && robots.map((r) => {
              ...
              return (
                <g key={`trail-g-${r.id}`}>
                  <polyline points={pts} fill="none" stroke={r.color} strokeOpacity={0.22}
                            strokeWidth={w1} strokeLinecap="round" strokeLinejoin="round" />
                  <polyline points={pts} fill="none" stroke={r.color} strokeWidth={w2}
                            strokeDasharray="0.4 1.2" strokeLinecap="round" />
                </g>
              );
            })}
          ...
          {MAP_LANDMARKS.map((l) => (
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] uppercase tracking-[0.14em] font-medium text-foreground/85
                                 bg-card/65 backdrop-blur-md px-2 py-0.5 rounded-full whitespace-nowrap
                                 border border-cyan-accent/15 shadow-sm">
                  {l.kind === "base"  && <Anchor    className="inline size-2.5 mr-1 -mt-0.5 text-cyan-accent" />}
                  {l.kind === "infra" && <Building2 className="inline size-2.5 mr-1 -mt-0.5 text-cyan-accent" />}
                  {l.label}
                </span>
              </div>
            </div>
          ...
          {clusters.map((c, i) => (
            <Tooltip key={i}>
                  ...
                  {c.type === "cluster" ? (
                    <div className="flex flex-col items-center">
                      <div className="relative">
                        ...
                        <div className="relative size-10 rounded-full flex items-center justify-center
                      ...
                      <span className="text-[9px] uppercase tracking-[0.1em] text-cyan-accent/80 mt-0.5 font-display">проб</span>
                    </div>
                  ) : (
                    <div className="relative">
                      {highlightedSampleId === c.sample.id && (
                        <span className="absolute left-1/2 top-1/2 size-9 rounded-full
                      ...
                      <div
                        ...
                        style={{
                          background: "radial-gradient(circle at 30% 30%, oklch(0.95 0.06 195), oklch(0.78 0.10 200) 65%, oklch(0.55 0.10 210))",
                          boxShadow: "0 0 8px oklch(0.82 0.09 195 / 0.85), inset 0 0 0 1px oklch(0.18 0.04 235 / 0.45)",
                        }}
                      />
                    </div>
                  )}
              ...
              <TooltipContent side="top" className="p-0 bg-card/95 backdrop-blur border-border max-w-[260px]">
                {c.type === "cluster" ? (
                  <div className="px-3 py-2 text-xs">
                    <div className="font-semibold text-foreground">Кластер: {c.count} проб</div>
                    <div className="text-muted-foreground mt-0.5">Приблизьте карту для детального просмотра</div>
                  </div>
                ) : (
                  (() => {
                    const s = c.sample;
                    ...
                    const oxOk = s.oxygen >= 6;
                    const turOk = s.turbidity <= 5;
                    const tempOk = s.temperature <= 24;
                    const pollTone =
                      s.pollution < 20 ? "text-success" :
                      s.pollution < 45 ? "text-warning" :
                      s.pollution < 70 ? "text-destructive" : "text-[oklch(0.78_0.18_300)]";
                    const pollLabel =
                      s.pollution < 20 ? "низкий" :
                      s.pollution < 45 ? "умеренный" :
                      s.pollution < 70 ? "высокий" : "критический";
                    return (
                      <div className="text-xs">
                        <div className="px-3 py-2 border-b border-border flex items-center justify-between gap-3">
                          <div className="font-semibold text-foreground">Проба {s.id.toUpperCase()}</div>
                        ...
                        <div className="px-3 py-2 grid grid-cols-2 gap-x-3 gap-y-1 tabular-nums">
                          <div className="flex justify-between"><span className="text-muted-foreground">pH</span>          <span className={phOk ? "text-cyan-accent" : "text-warning"}>{s.ph}</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">O₂</span>          <span className={oxOk ? "text-success"    : "text-warning"}>{s.oxygen} мг/л</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">Мутность</span>    <span className={turOk ? "text-foreground"  : "text-warning"}>{s.turbidity} NTU</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">T°</span>          <span className={tempOk ? "text-foreground" : "text-warning"}>{s.temperature}°C</span></div>
                          <div className="flex justify-between col-span-2"><span className="text-muted-foreground">Глубина</span>     <span>{s.depth} м</span></div>
                          <div className="flex justify-between col-span-2"><span className="text-muted-foreground">Загрязнение</span> <span className={pollTone}>{s.pollution}/100 · {pollLabel}</span></div>
                        </div>
              ...
              <Tooltip key={r.id}>
                <TooltipTrigger asChild>
                  ...
                  >
                    <div className="relative">
                      {/* outer halo */}
                      {active && (
                        <span className="absolute left-1/2 top-1/2 size-14 rounded-full marker-halo"
                              style={{ background: `radial-gradient(circle, ${r.color} 0%, transparent 65%)`, opacity: 0.35 }} />
                      )}

                      {/* heading wedge arc behind the badge */}
                      {active && (
                        <svg className="absolute -inset-2 pointer-events-none" viewBox="-50 -50 100 100"
                      ...
                      <div className={`absolute -bottom-1 -right-1 size-3 rounded-full border-2 border-card ${
                        ...
                        r.status === "rtl" ? "bg-warning pulse-dot" :
                        "bg-muted-foreground"
                      }`} />
                    </div>
                ...
                <TooltipContent side="top" className="bg-card/95 backdrop-blur border-border">
                  <div className="text-xs space-y-0.5">
                    ...
                    {active && <div className="text-success">{r.status === "mission" ? "Выполняет миссию" : "В сети"} · {r.battery.toFixed(0)}% · {r.speed} м/с</div>}
          ...
          {editMode && draftWaypoints && draftWaypoints.map((w, i) => (
            ...
            >
              <div className="size-6 rounded-full bg-cyan-accent text-background border-2 border-background
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-1 glass-panel rounded-xl p-1">
          <ToolBtn label="Приблизить (Wheel ↑)"  onClick={() => { const n = Math.min(6, zoom + 0.6); setZoom(n); setPan((p) => clampPan(p, n)); }}><ZoomIn className="size-4" /></ToolBtn>
          <ToolBtn label="Отдалить (Wheel ↓)"    onClick={() => { const n = Math.max(1, zoom - 0.6); setZoom(n); setPan((p) => clampPan(p, n)); }}><ZoomOut className="size-4" /></ToolBtn>
          <ToolBtn label="Сбросить вид"          onClick={() => { setZoom(1.4); setPan({ x: 0, y: 0 }); }}><RotateCcw className="size-4" /></ToolBtn>
          <ToolBtn label="Вписать"               onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}><Maximize2 className="size-4" /></ToolBtn>
          <div className="h-px bg-cyan-accent/15 my-0.5" />
          <ToolBtn label={showTrails ? "Следы: вкл" : "Следы: выкл"} active={showTrails} onClick={() => setShowTrails((s) => !s)}><Waves className="size-4" /></ToolBtn>
          <ToolBtn label={showGrid ? "Сетка: вкл" : "Сетка: выкл"}   active={showGrid}   onClick={() => setShowGrid((s) => !s)}><Layers className="size-4" /></ToolBtn>
        </div>
        ...
        <div className="absolute top-4 right-4 flex flex-col items-end gap-2">
          <div className="glass-panel rounded-xl p-2 flex items-center gap-2">
            <div className="relative size-9">
              ...
              <div className="absolute left-1/2 top-0.5 -translate-x-1/2 text-[8px] text-cyan-accent font-bold">N</div>
              <div className="absolute left-1/2 bottom-0.5 -translate-x-1/2 text-[8px] text-muted-foreground">S</div>
          ...
          {robots.map((r) => (
            <div key={r.id} className="flex items-center gap-2">
              ...
              <span className={`ml-auto text-[10px] font-mono ${
                r.status === "online" || r.status === "mission" ? "text-success" :
                r.status === "rtl" ? "text-warning" : "text-muted-foreground"
              }`}>
                {r.status === "online" ? "online" :
                 r.status === "mission" ? "mission" :
                 r.status === "rtl" ? "RTL" : "offline"}
              </span>
            </div>
          ))}
          <div className="h-px bg-cyan-accent/15 my-1.5" />
          <div className="flex items-center gap-2">
            <div className="size-2.5 rounded-full sample-pulse"
                 style={{ background: "radial-gradient(circle at 30% 30%, oklch(0.95 0.06 195), oklch(0.78 0.10 200))" }} />
            Точка пробы
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="absolute bottom-4 right-4 glass-panel rounded-xl px-3 py-2 text-[10px] font-mono">
          ...
          <div className="flex justify-between w-20 mt-0.5 text-muted-foreground">
            <span>0</span><span>5</span><span>10 км</span>
          </div>
        </div>
      </div>
    </TooltipProvider>
...
function ToolBtn({
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side="right" className="bg-card/95 backdrop-blur border-border text-xs">{label}</TooltipContent>
    </Tooltip>
  );
}
