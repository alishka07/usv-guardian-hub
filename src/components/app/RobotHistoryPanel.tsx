...
export function RobotHistoryPanel({
  ...
  const robotLog = useMemo(
    () => log.filter((e) => e.robotId === robot.id).slice(0, 20),
    [log, robot.id],
  );
  const robotSamples = useMemo(
    () =>
      samples
        .filter((s) => s.robotId === robot.id)
  ...
  return (
    <div className="absolute left-4 top-4 bottom-4 w-[320px] z-20 bg-card/95 backdrop-blur-xl border border-border rounded-xl shadow-2xl overflow-hidden hidden lg:flex flex-col animate-in slide-in-from-left-4 fade-in duration-300">
      <div
      ...
      <div className="overflow-y-auto flex-1 divide-y divide-border">
        ...
        <div className="grid grid-cols-2 gap-2 p-4">
          ...
          <div className="rounded-lg bg-panel/50 border border-border p-2.5">
            ...
            <div className="text-2xl font-bold tabular-nums mt-0.5 text-cyan-accent">
              {robotSamples.length}
            </div>
          </div>
        </div>
            ...
            <ol className="relative space-y-2 pl-4 before:absolute before:left-[5px] before:top-1 before:bottom-1 before:w-px before:bg-border">
              {robotLog.map((e) => (
                <li key={e.id} className="relative">
                  <span
                    className={`absolute -left-[15px] top-1.5 size-2.5 rounded-full ring-2 ring-card ${SEV_DOT[e.severity]}`}
                  />
        ...
        <div className="p-4 space-y-2">
          <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.12em] text-muted-foreground font-semibold">
            <span className="flex items-center gap-1.5">
              <Droplets className="size-3" /> Последние пробы
            </span>
          ...
          {robotSamples.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {robotSamples.map((s) => {
                ...
                const phWarn = s.ph < thresholds.ph.min || s.ph > thresholds.ph.max;
