  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard
          label="На связи"
          value={robots.filter((r) => r.status !== "offline").length}
          accent="success"
        />
        <StatCard
          label="Не в сети"
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[720px]">
            ...
            {robots.map((r) => (
              <div
                key={r.id}
                className="grid grid-cols-[1fr_120px_140px_140px_120px_120px] gap-4 px-5 py-4 border-b border-border last:border-b-0 items-center hover:bg-panel/40 transition"
              >
                ...
                <div>
                  <Badge className={STATUS_META[r.status].badge}>
                    <span
                      className={`size-1.5 rounded-full mr-1.5 ${STATUS_META[r.status].dot} ${STATUS_META[r.status].pulse ? "pulse-dot" : ""}`}
                    />
                ...
                <div className="flex justify-end">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onOpenRobot?.(r)}
                    title="Открыть на карте"
                    className="gap-1.5 text-xs"
                  >
                    <Settings2 className="size-4" /> Управление
                  </Button>
                </div>
              </div>
            ))}
...
const SEV_STYLE = {
  ...
  critical: {
    ...
    border: "border-[oklch(0.55_0.22_300)]/30",
    dot: "bg-[oklch(0.7_0.2_300)]",
    label: "Критично",
  },
} as const;
  ...
  return (
    <Card className="bg-card border-border p-0 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[11px] text-muted-foreground">
              Системные события USV в реальном времени
            </div>
          </div>
        </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((e) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="grid grid-cols-[140px_1fr_180px_140px] gap-4 px-5 py-3 border-b border-border last:border-b-0 items-center hover:bg-panel/40 transition group"
                  >
                    ...
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`size-2 rounded-full ${s.dot} ${e.severity === "danger" || e.severity === "critical" ? "pulse-dot" : ""}`}
                      />
                    ...
                    <div>
                      <Badge
                        variant="outline"
                        className={`${s.bg} ${s.text} ${s.border} text-[10px] uppercase tracking-wider font-semibold`}
                      >
                        {s.label}
                      </Badge>
                    </div>
      ...
      <div className="px-5 py-2.5 border-t border-border text-[11px] text-muted-foreground flex items-center justify-between bg-panel/30">
        <span>
          Всего: <span className="font-mono text-foreground">{filtered.length}</span> / {log.length}
        </span>
        <span className="flex items-center gap-1.5">
          <span className="size-1.5 rounded-full bg-success pulse-dot" />
          Поток событий активен
        </span>
      </div>
    </Card>
  );
